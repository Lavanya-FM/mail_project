...
export default function P2PTransferProgress({
  ...
  // Calculate overall progress only from files that are still transferring
  // Delivered files (100%) should not skew the average
  const activeFiles = files.filter(f => f.status !== 'delivered' && f.status !== 'failed');
  const totalProgress = activeFiles.length > 0
    ? activeFiles.reduce((sum, f) => sum + f.progress, 0) / activeFiles.length
    : files.every(f => f.status === 'delivered') ? 100 : 0;
