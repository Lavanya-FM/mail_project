...
export default function SuperAdminDashboard() {
    ...
    const loadDashboardData = async () => {
        setLoading(true);
        try {
            const [statsData, newUsersData, highCarbonData, storageData, p2pData, logsData] = await Promise.all([
                ...
                superadminService.getSystemLogs()
            ]);
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-4">
                            <button
                                ...
                                className={`px-4 py-2 rounded-xl font-medium transition-all ${activeTab === 'overview' ? 'bg-blue-600 text-white' : 'text-gray-600 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800'}`}
                            >
                                Overview
                            </button>
                            <button
                                onClick={() => setActiveTab('logs')}
                                className={`px-4 py-2 rounded-xl font-medium transition-all ${activeTab === 'logs' ? 'bg-blue-600 text-white' : 'text-gray-600 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800'}`}
                            >
                                System Logs
                            </button>
                        </div>
                ...
                {activeTab === 'overview' ? (
                    ...
                    <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-2xl shadow-2xl border border-gray-200/50 dark:border-gray-700/50 p-6 hover:shadow-3xl transition-all duration-300">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <div>
                                <h2 className="text-xl font-bold bg-gradient-to-r from-red-600 to-pink-600 dark:from-red-400 dark:to-pink-400 bg-clip-text text-transparent">
                                    System Error Logs
                                </h2>
                                <p className="text-sm text-gray-600 dark:text-gray-400">Real-time application health monitoring</p>
                            </div>
                        </div>
                        <div className="overflow-x-auto">
                            <table className="w-full">
                                <thead>
                                    <tr className={`border-b-2 ${theme === 'dark' ? 'border-gray-700' : 'border-gray-200'}`}>
                                        <th className={`text-left py-4 px-4 font-semibold ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>Time</th>
                                        <th className={`text-left py-4 px-4 font-semibold ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>Level</th>
                                        <th className={`text-left py-4 px-4 font-semibold ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>Message</th>
                                        <th className={`text-left py-4 px-4 font-semibold ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>IP / User Agent</th>
                                        <th className={`text-left py-4 px-4 font-semibold ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>Details</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {systemLogs.map((log) => (
                                        <tr key={log.id} className={`border-b ${theme === 'dark' ? 'border-gray-700/50 hover:bg-gray-700/30' : 'border-gray-100 hover:bg-red-50/50'} transition-all duration-200`}>
                                            <td className={`py-4 px-4 text-xs ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>
                                                {new Date(log.created_at).toLocaleString()}
                                            </td>
                                            <td className="py-4 px-4">
                                                <span className={`px-2 py-1 rounded text-xs font-bold uppercase ${log.level === 'error' ? 'bg-red-100 text-red-600' : 'bg-yellow-100 text-yellow-600'}`}>
                                                    {log.level}
                                                </span>
                                            </td>
                                            <td className={`py-4 px-4 font-medium ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
                                            ...
                                            <td className={`py-4 px-4 text-xs ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>
                                                <div className="font-bold">{log.ip}</div>
                                                <div className="truncate max-w-xs">{log.user_agent}</div>
                                            </td>
                                            <td className="py-4 px-4">
                                                <button
                                                    onClick={() => alert(JSON.stringify(JSON.parse(log.context || '{}'), null, 2))}
